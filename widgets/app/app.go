...
func getAppFile() (string, error) {
	...
	file = filepath.Join(string(os.PathSeparator), "app.ssk")
	if has, _ := application.App.Exists(file); has {
		return file, nil
	}
...
func exportAPI() error {
	...
	// load apis
	api.LoadSource("<widget.app>.ssk", source, "widgets.app")
...
func processXgen(process *process.Process) interface{} {
	...
	if admin, has := login.Logins["admin"]; has {
		...
		layout["cover"] = "https://cdn.pixabay.com/photo/2023/11/10/20/32/duck-8380065_960_720.jpg"
	...
	if user, has := login.Logins["user"]; has {
		...
		layout["cover"] = "https://cdn.pixabay.com/photo/2023/11/05/15/24/autumn-8367628_960_720.jpg"
